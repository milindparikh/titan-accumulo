...
public class AccumuloStoreDriver {
	...
	AccumuloJavaOrderedKeyColumnValueStore accumuloJavaOrderedKeyColumnValueStore = accumuloJavaStoreManager.openDatabase("hellotable2");
	
	StoreTxConfig config = new StoreTxConfig();
	StoreTransaction st = accumuloJavaStoreManager.beginTransaction(config);
	accumuloJavaOrderedKeyColumnValueStore.delete(new StaticByteBuffer("row9".getBytes()), st);
	...
	accumuloJavaOrderedKeyColumnValueStore.insert(new StaticByteBuffer("row1".getBytes()), new StaticByteBuffer("val1".getBytes()), st);
	accumuloJavaOrderedKeyColumnValueStore.insert(new StaticByteBuffer("row8".getBytes()), new StaticByteBuffer("val1".getBytes()), st);
	accumuloJavaOrderedKeyColumnValueStore.insert(new StaticByteBuffer("row9".getBytes()), new StaticByteBuffer("val1".getBytes()), st);
	...
	StaticBuffer buf = accumuloJavaOrderedKeyColumnValueStore.get(new StaticByteBuffer("row9".getBytes()), st);
	String str = new String(buf.asByteBuffer().array(), Charset.forName("UTF-8"));
	...
	RecordIterator<KeyValueEntry> iter = accumuloJavaOrderedKeyColumnValueStore.getSlice(startB, endB, selectAll, st);
	...
	System.out.println(accumuloJavaOrderedKeyColumnValueStore.containsKey(new StaticByteBuffer("row9".getBytes()), st));
