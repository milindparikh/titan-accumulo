...
public class AccumuloJavaOrderedKeyColumnValueStore implements OrderedKeyValueStore {
	...
	try {
	    ...
	    String startKey = new String (key.asByteBuffer().array(), Charset.forName("UTF-8") );
	    accumuloConnector = storeManager.getAccumuloConnector();
	    Connector connector = accumuloConnector.getConnector();
	    Scanner scanner = connector.createScanner(tableName, accumuloConnector.auths);
		...
		return new StaticByteBuffer(entry.getValue().toString().getBytes());
	    }
	    else {
		return null;
	    }
    ...
    @Override
	public boolean containsKey(StaticBuffer key, StoreTransaction txh) throws 
	    StorageException {
	...
	// no support for localkeypartiton because features.hasLocalKeyPartition = false
        throw new UnsupportedOperationException();
    }

    @Override
    public RecordIterator<KeyValueEntry> getSlice(StaticBuffer keyStart, StaticBuffer keyEnd,
                                                  final KeySelector selector, StoreTransaction txh) throws StorageException {
	...
	try {
	    ...
	    String startKey = new String (keyStart.asByteBuffer().array(), Charset.forName("UTF-8") );
	    String endKey = new String (keyEnd.asByteBuffer().array(), Charset.forName("UTF-8") );
		...
		@Override
			...
			key =  new StaticByteBuffer(entry.getKey().toString().getBytes());
			value =  new StaticByteBuffer(entry.getValue().toString().getBytes());
	...
	try {
	    ...
	    String startKey = new String (key.asByteBuffer().array(), Charset.forName("UTF-8") );
	    String startValue = new String (value.asByteBuffer().array(), Charset.forName("UTF-8") );
	    ...
	    m.put(cf, cq, cv, new Value(startValue.getBytes(Charset.forName("UTF-8"))));
